#!/usr/bin/env nbb
;; verify-custody.cljs — この repo が「与えられたもの」から動いていないことを検査する。
;;
...
;; その subtree を GitHub から取り、**path と blob SHA の集合**が
;; 「tracked − :canonical-files」と一致することを見る。バイト数ではなく SHA を
;; 見るのは、同じ長さの別内容を通さないため。
;;
...
(def ^:private local
  (let [rows (->> (str/split-lines (run "git" ["ls-files" "-s"]))
    ...
    (into {} (remove (fn [[p _]] (canonical p)) rows))))
...
(def ^:private only-local    (sort (remove #(contains? upstream %) (keys local))))
(def ^:private only-upstream (sort (remove #(contains? local %) (keys upstream))))
(def ^:private changed
  (sort (for [[p sha] local
              :when (and (contains? upstream p) (not= sha (get upstream p)))]
          p)))
...
(report "upstream に在るのに手元に無い file" only-upstream)
(report "path は同じで内容が違う file" changed)

(if (and (empty? only-local) (empty? only-upstream) (empty? changed))
  (do (println (str "PASS — " (count local) " blob が etzhayyim/root@" (subs rev 0 7)
                    " の " path " と path/SHA まで一致"))
      (js/process.exit 0))
  (do (println "FAIL — custody が動いている。追加なら :canonical-files に申告すること")
      (js/process.exit 1)))
