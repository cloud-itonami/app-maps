(ns maps.tile.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `maps.tile.route/dispatch`
  が決め、ページの中身は `maps.tile.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  `appview/maps-tile-server-t1l3srv0/wrangler.jsonc` の `main` は
  `../../dist/worker.js` を指し、それはこの名前空間をコンパイルしたものである。
  移行前は `svelte/.svelte-kit/cloudflare/_worker.js`（tree に存在しない
  SvelteKit のビルド出力）を指していた。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [maps.tile.route :as route]
...
(defn- relay-headers
  "移行前の `+server.ts` と同じ組み立て: **client の header を引き継ぎ**、
  `host` を落とし、content-type と 2 つの `x-etzhayyim-*` を上書きする。

  ⚠ この転送は client の `authorization` / `cookie` も上流へ渡す。移行前から
  そうなっており、**移行では直していない** —— 中継先の header 方針は移行とは
  別の決定である（docs/adr/0001 の『持ち越した欠陥』）。`x-etzhayyim-bff` の値
  だけは `sveltekit-edge-bff` から変えた。SvelteKit ではないものが SvelteKit を
  名乗るのは、直せる嘘だからである。"
  [req nsid]
  (let [h (js/Headers. (.-headers req))]
    (.delete h "host")
    (.set h "content-type" "application/json")
    (.set h "x-etzhayyim-bff" "cljs-edge-bff")
...
(defn- proxy-xrpc
  ...
  (let [url (route/mcp-router-url (env->map env))
        headers (relay-headers req nsid)]
    (-> (.json req)
        (.catch (fn [_] #js {}))
        (.then
         (fn [input]
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "maps-tile-server" :runtime "cljs"
